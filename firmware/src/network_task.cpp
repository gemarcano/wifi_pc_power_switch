	...
	for (;;)
	{
		...
		do
		{
			...
			if (err != 0)
			{
				sys_log.push(
					std::format(
						"unable to listen on server, error {}", strerror(err)
					)
				);
			}
		}
		...
		for (;;)
		{
			...
			if (!accept_result)
			{
				sys_log.push(
					std::format(
						"unable to accept socket, error {}",
						strerror(accept_result.error())
					)
				);
				// FIXME what if the error is terminal? Are there any terminal
				// errors?
				continue;
			}
			...
			if (request_result)
			{
				...
				if (amount < 8)
				{
					auto explanation = std::format(
						"Received bad network request with size {}", amount
					);
				...
				{
					auto explanation = std::format(
						"Received bad network request, bad magic {}", magic
					);
					sys_log.push(explanation);
					handler.send(explanation);
					continue;
				...
				case 0: // toggle, an additional 4 byte field
				{
					...
					{
						auto explanation = std::format(
							"Received bad network request, bad size {}", amount
						);
						sys_log.push(explanation);
						handler.send(explanation);
						continue;
					...
					time = ntoh(time);
					auto explanation =
						std::format("Received network toggle request {}", time);
					sys_log.push(explanation);
					handler.send(explanation);
				...
				case 1:
				{
					...
					auto explanation =
						std::format("boot select: {}", pcrb::get_boot_select());
					sys_log.push(explanation);
					handler.send(explanation);
					break;
				}
				case 2:
				{
					...
					select = ntoh(select);
					auto explanation = std::format(
						"Received boot select request {}, ", select
					);
					sys_log.push(explanation);
					handler.send(explanation);
				...
				case 3:
				{
					...
					auto explanation = std::format(
						"PC 3.3V rail status: {}", pcrb::current_pc_state()
					);
					sys_log.push(explanation);
					handler.send(explanation);
					break;
				...
				{
					auto explanation = std::format(
						"Received bad network request, unknown command {}",
						request
					);
					sys_log.push(explanation);
					handler.send(explanation);
			...
			{
				const char *err = request_result.error() == EAGAIN
									  ? "timeout"
									  : strerror(request_result.error());
				auto explanation =
					std::format("failed to handle request: {}", err);
				sys_log.push(explanation);
				handler.send(explanation);
			}
		}
